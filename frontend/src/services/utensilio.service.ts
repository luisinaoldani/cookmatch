...
export const getUtensilioById = async (id: number): Promise<Utensilio | undefined> => {
  await delay(300);
...
export const createUtensilio = async (newItem: Partial<Utensilio>): Promise<Utensilio> => {
  await delay(300);
  const utensilio = { ...newItem, id: nextId++ } as Utensilio;
  data.push(utensilio);
  return utensilio;
};

export const updateUtensilio = async (id: number, changes: Partial<Utensilio>): Promise<Utensilio | undefined> => {
  await delay(300);
